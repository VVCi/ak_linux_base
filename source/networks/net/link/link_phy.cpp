...
/* serail interface */
#define LINK_PHY_DEV_PATH			"/dev/ac_mod_com"
...
void* link_phy_rx_thread_handler(void*) {
	...
	while(1) {
		rx_read_len = read(link_phy_fd, rx_buffer, LINK_PHY_RX_THREAD_BUFFER_SIZE);
		if (rx_read_len > 0) {
			link_phy_rev_frame_parser(rx_buffer, rx_read_len);
		}
	}
...
void fsm_link_phy_state_init(ak_msg_t* msg) {
	...
	case GW_LINK_PHY_INIT: {
		...
		task_post_pure_msg(GW_LINK_MAC_ID, GW_LINK_MAC_PHY_LAYER_STARTED);
	}
		break;
...
void link_phy_frame_send_max_retry() {
	link_phy_send_state_set(LINK_PHY_SEND_STATE_IDLE);
	task_post_pure_msg(GW_LINK_MAC_ID, GW_LINK_MAC_FRAME_SEND_ERR);
}

void link_phy_frame_send() {
	if (link_phy_send_state_get() == LINK_PHY_SEND_STATE_SENDING) {
		link_phy_frame_write(&send_link_phy_frame);
		timer_set(GW_LINK_PHY_ID, GW_LINK_PHY_FRAME_SEND_TO, LINK_PHY_FRAME_SEND_TO_INTERVAL, TIMER_ONE_SHOT);
	}
}
