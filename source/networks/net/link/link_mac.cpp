...
static void task_link_mac(ak_msg_t* msg);
...
void fsm_link_mac_state_handle(ak_msg_t* msg) {
	...
	case GW_LINK_MAC_FRAME_SEND_REQ: {
		...
		if (link_mac_send_state_get() == LINK_MAC_SEND_STATE_IDLE) {
			...
			task_post_common_msg(GW_LINK_MAC_ID, GW_LINK_MAC_FRAME_SEND_START, (uint8_t*)&pdu_id, sizeof(uint32_t));
		}
		else {
	...
	case GW_LINK_MAC_FRAME_SEND_DONE: {
		...
		if (link_mac_send_state_get() == LINK_MAC_SEND_STATE_SENDING) {
			if (++link_mac_frame_send.header.fidx < link_mac_frame_send.header.fnum) {
				...
				task_post_common_msg(GW_LINK_ID, GW_LINK_SEND_DONE, (uint8_t*)&link_pdu_send_done, sizeof(uint32_t));
			}
		}
	}
		break;
	...
	case GW_LINK_MAC_FRAME_SEND_ERR: {
		...
		if (link_mac_pdu_sending_retry_counter++ < LINK_MAC_PDU_SENDING_RETRY_COUNTER_MAX) {
			...
			task_post_common_msg(GW_LINK_ID, GW_LINK_SEND_ERR, (uint8_t*)&send_pdu_err, sizeof(uint32_t));
		}
	}
		break;
...
void link_mac_frame_send_end() {
	if (fifo_availble(&link_pdu_id_fifo)) {
		...
		task_post_common_msg(GW_LINK_MAC_ID, GW_LINK_MAC_FRAME_SEND_START, (uint8_t*)&link_pdu_send, sizeof(uint32_t));
	}
	else {
		link_mac_send_state_set(LINK_MAC_SEND_STATE_IDLE);
	}
...
void link_mac_frame_send_req() {
	if (link_mac_send_state_get() == LINK_MAC_SEND_STATE_SENDING) {
		task_post_common_msg(GW_LINK_PHY_ID, GW_LINK_PHY_FRAME_SEND_REQ, (uint8_t*)&link_mac_frame_send, sizeof(link_mac_frame_t));
